...
bool init_chip8(chip8_t *chip8, const char *rom_name)
{
    ...
    // Get Rom using SDL (Can be used on adroid)
    SDL_RWops *rom = SDL_RWFromFile(rom_name, "rb");
    if (!rom)
    {
        SDL_Log("ROM %s is not valid: %s\n", rom_name, SDL_GetError());
        return false;
    }

    const Sint64 rom_size = SDL_RWsize(rom);
    const size_t max_size = sizeof(chip8->ram) - ENTRY_POINT;

    if (rom_size < 0)
    {
        SDL_Log("Could not determine size of ROM %s\n", rom_name);
        SDL_RWclose(rom);
        return false;
    }
    if ((size_t)rom_size > max_size)
    {
        SDL_Log("ROM %s is too big (size %lld) for RAM (size %zu)\n", rom_name, (long long)rom_size, max_size);
        SDL_RWclose(rom);
        return false;
    }
    // Load Rom
    if (SDL_RWread(rom, &chip8->ram[ENTRY_POINT], 1, (size_t)rom_size) != (size_t)rom_size)
    {
